    ...
    handlers: {
      POST: async ({ request }) => {
        if (!hasSupabaseAdminConfig())
          return Response.json({ error: "Opinie nie są jeszcze skonfigurowane." }, { status: 503 });
        const token = request.headers.get("authorization")?.replace(/^Bearer\s+/i, "");
        if (!token)
          return Response.json({ error: "Zaloguj się, aby wystawić opinię." }, { status: 401 });
        const admin = getSupabaseAdmin();
        const { data: user, error: userError } = await admin.auth.getUser(token);
        if (userError || !user.user)
          return Response.json({ error: "Sesja wygasła." }, { status: 401 });
        try {
          await enforceRateLimit(admin, "review", user.user.id, 10, 3600);
        } catch (cause) {
          return Response.json(
            {
              error:
                cause instanceof RateLimitExceededError
                  ? cause.message
                  : "Opinie są chwilowo niedostępne.",
            },
        ...
        if (!parsed.success)
          return Response.json({ error: "Ocena musi mieć od 1 do 5 gwiazdek." }, { status: 400 });
        const { data: order } = await admin
          .from("orders")
          .select("id,buyer_id,seller_id,status")
          .eq("id", parsed.data.orderId)
          .maybeSingle();
        if (!order || order.buyer_id !== user.user.id || order.status !== "delivered")
          return Response.json(
            { error: "Opinię można wystawić po potwierdzonym odbiorze." },
            { status: 409 },
          );
          ...
          return Response.json(
            {
              error:
                error.code === "23505"
                  ? "Opinia do tego zamówienia już istnieje."
