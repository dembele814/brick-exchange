...
begin
  ...
  if found then
    if purchase.buyer_id <> p_buyer_id or purchase.status <> 'pending_payment'
      ...
      or purchase.stripe_livemode is distinct from p_stripe_livemode then
      raise exception 'Listing reserved or previous checkout requires reconciliation';
    end if;
    -- Repeated clicks by the same buyer reuse the one open Stripe session. The
    -- immutable delivery snapshot from the first attempt prevents two active
    -- sessions and a possible double payment for one listing.
